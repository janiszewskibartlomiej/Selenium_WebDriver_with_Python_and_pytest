...
@pytest.mark.parametrize("user, password, enter_key, login_by_facebook", [
    (test_data["user_name"], test_data["password"], True, False),  # login by user name, submit enter
    (test_data["user_name"], test_data["password"], False, False),  # login by user name, submit click
    (test_data["user_email"], test_data["password"], True, False),  # login by email, submit enter
    (test_data["user_email"], test_data["password"], False, False),  # login by email, submit click
    (test_data["user_email"].upper(), test_data["password"], True, False),  # login by email - upper case, submit enter
    (test_data["user_email"].upper(), test_data["password"], False, False),  # login by email - upper case, submit click
    ...
    (test_data["user_email"], test_data["password"], False, True)  # login by facebook

])
@pytest.mark.login
def test_TS01_successful_login(request, driver, user, password, enter_key, login_by_facebook):
    try:
        login_page = LoginPage(driver=driver)
        login_page.assert_path_in_current_url(path=txt.LOGIN_ENDPOINT)
        username_input = login_page.get_element(by_locator=LoginPageLocators.USERNAME_FIELD)
        assert username_input.tag_name == txt.INPUT_TAG
        password_input = login_page.get_element(by_locator=LoginPageLocators.PASSWORD_FIELD)
        assert password_input.tag_name == txt.INPUT_TAG

        if login_by_facebook:
            login_page.click_on_and_wait_for_a_new_page(by_loctor=LoginPageLocators.LOGIN_BY_FACEBOOK)
            login_page.enter_text(by_locator=LoginPageLocators.FACEBOOK_EMAIL, text=test_data["facebook_email"])
            login_page.enter_text(by_locator=LoginPageLocators.FACEBOOK_PASSWORD,
                                  text=test_data["facebook_password"])
            login_page.click_on_and_wait_for_a_new_page(by_loctor=LoginPageLocators.FACEBOOK_LOGIN_BUTTON)
        else:
        ...
        drop_down = login_page.get_element(by_locator=LoginPageLocators.DROP_DOWN_SECTION)
        assert txt.LOGIN_INFORMATION_IN_DROPDOWN in drop_down.get_attribute("innerHTML")
        login_page.assert_element_text_in_page_source(element_text=txt.LOGIN_INFORMATION_IN_DROPDOWN)
